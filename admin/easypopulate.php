...
//        header("Content-type: text/x-csv");
    ...
    while ($row) {
        ...
        $row['цена_диллер'] = round($row['цена_дилер'], 0);
...
<table border="0" width="100%" cellspacing="2" cellpadding="2">
    <tr>
        ...
        <td class="pageHeading" valign="top"><?php echo PAGE_HEADER; ?>
            ...
            <form name="clear_form" action="easypopulate.php<?php if (defined('SID') && tep_not_null(SID)) {
        ...
        ?>
            ...
            <form name="clear_form2" action="easypopulate.php<?php if (defined('SID') && tep_not_null(SID)) {
                echo '?' . tep_session_name() . '=' . tep_session_id();
        ...
        ?>
            ...
            <table width="100%" cellpadding="5" cellspacing="0" style="border-collapse:collapse;">
                <tr>
                    <td width="75%"
                        style="border-style:solid; border-width:thin; border-color:#CCCCCC;vertical-align: top;">
                        ...
                        <form enctype="multipart/form-data"
                              action="easypopulate.php?split=0<?php if (defined('SID') && tep_not_null(SID)) {
                                  echo '&' . tep_session_name() . '=' . tep_session_id();
                            ...
                            <p style="margin-top: 0px;">
                                ...
                                <select name="imput_mode">
                                    ...
                                    <option value="update_kyiv">Обновить склад Киев</option>
                                </select>
...
//        $filelayout['картинка_большая'] = $iii++;
            ...
            p.products_internal_sku as код,
            p.products_images as картинки,
            p.products_quantity as колич,
            p.kiev_stock as киев,
            p.our_stock as наш,
            p.mankovka_stock as манк,
            $ep_additional_layout_product_select
            p.products_price_2 as цена_дилер,
...
function process_row($item1, $filelayout, $filelayout_count, $default_these, $ep_separator, $languages, $custom_fields, $leng, $checkArttributes)
{
    ...
    $v_products_quantity = (!empty($items[$filelayout['колич']]) ? $items[$filelayout['колич']] : '0');
    $v_products_quantity_kiev = (!empty($items[$filelayout['киев']]) ? $items[$filelayout['киев']] : '0');
    $v_products_quantity_our = (!empty($items[$filelayout['наш']]) ? $items[$filelayout['наш']] : '0');
    $v_products_quantity_mank = (!empty($items[$filelayout['манк']]) ? $items[$filelayout['манк']] : '0');
    $v_products_price_dealer = (!empty($items[$filelayout['цена_дилер']]) ? round($items[$filelayout['цена_дилер']], 0) : '0');
    $v_products_model = $items[$filelayout['код']];

//    $v_products_model_barcode = (!empty($items[$filelayout['штрихкод']]) ? $items[$filelayout['штрихкод']] : '0');

    $v_products_model_barcode = $items[$filelayout['штрихкод']];
...
//        $v_products_id    =    $items[$filelayout['v_products_id']];
        ...
        $sql = "SELECT p.products_id as v_products_id FROM " . TABLE_PRODUCTS . " as p WHERE p.products_internal_sku = '" . $v_products_model . "'";
        ...
        // Get the ID
        $sql = "SELECT p.products_id as v_products_id    FROM " . TABLE_PRODUCTS . " as p WHERE p.products_internal_sku = '" . $items[$filelayout['код']] . "'";
        $result = tep_db_query($sql);
        $row = tep_db_fetch_array($result);
        ...
        $sql = "SELECT
                ...
                FROM
                    ...
                    " . TABLE_PRODUCTS_TO_CATEGORIES . " as ptoc
                WHERE
                    p.products_internal_sku = '" . $items[$filelayout['код']] . "' AND
                    p.products_id = ptoc.products_id AND
                    ptoc.categories_id = subc.categories_id
                LIMIT 1
            ";
        ...
        if ((sizeof($row) > 1) && ($_POST['imput_mode'] == "normal" || $_POST['imput_mode'] == "update" || $_POST['imput_mode'] == "addnew" || $_POST['imput_mode'] == 'update_kyiv')) {
        ...
        if ($process_product == true || $_POST['imput_mode'] == 'update_kyiv') {
            ...
            if ($v_products_model != "" || $v_products_model_barcode!="") {
                ...
                if ($v_products_model != "" ) {
                    $result = tep_db_query("SELECT products_id FROM " . TABLE_PRODUCTS . " WHERE (products_internal_sku = '" . $v_products_model . "')");
                }else if($v_products_model_barcode!=""){
                    $result = tep_db_query("SELECT products_id FROM " . TABLE_PRODUCTS . " WHERE (products_barcode = '" . $v_products_model_barcode . "')");
                }
                // First we check to see if this is a product in the current db.

                if (tep_db_num_rows($result) == 0 && $_POST['imput_mode'] == 'addnew') {
                    ...
                    // End: Support for other contributions
                    // /////////////////////////////////////////////////////////////////////
                    if ($v_products_price == null) {
                        $v_products_price_fields = '';
                        $v_products_price_status_data = '';
                    } else {
                        $v_products_price_fields = 'products_price,';
                        $v_products_price_status_data = "'$v_products_price',";
                    }

                    $query = "INSERT INTO " . TABLE_PRODUCTS . " (
                                products_images,
                                $ep_additional_fields
                                products_internal_sku,
                                $v_products_price_fields
                                products_price_2,
                              ...
                              VALUES (
                                ...
                                '$v_products_weight',
                                '$v_products_quantity',
                                '$v_products_quantity_kiev',
                                '$v_products_quantity_our',
                                '$v_products_quantity_mank',
                ...
                if (tep_db_num_rows($result) == 0 && $_POST['imput_mode'] == 'addnew') {
                    ...
                    while ($row = tep_db_fetch_array($result)) {
                        ...
                        if ($_POST['imput_mode'] == 'update_kyiv') {
                            echo "<font color='black'> Kyiv store updated</font><br />";
                        } else {
                        ...
                        if (!empty($v_products_price) || $v_products_price != 0) {
                            $v_products_price_status_upd = "products_price='" . $v_products_price . "',
                                            ";
                        }

                        // Оновлюємо магазин Київ по штрихкоду
                        if ($_POST['imput_mode'] == 'update_kyiv') {
                            // Для видачі штрихкодів по коду
//                            $updatequery = "UPDATE `products` SET `products_barcode`='" . $v_products_model_barcode . "' WHERE `products_internal_sku`='" . $v_products_model . "'";
//                            $query = $updatequery;
                            if($v_products_price=='0'){
                                $v_products_price='0.00';
                                var_dump($v_products_price);
                            }
                            if($v_products_quantity=='0'){
                                $v_products_quantity='0.00';
                            }
                            if($v_products_quantity_kiev=='0'){
                                $v_products_quantity_kiev='0.00';
                            }
                            if(empty($v_products_price) and empty($v_products_quantity) and empty($v_products_quantity_kiev)){
                                return false;
                            }
                            else if (empty($v_products_price)) {

                                if (empty($v_products_quantity_kiev)){
                                    $updatequery = "UPDATE `products` SET `products_quantity`='" . $v_products_quantity . "' WHERE `products_barcode`='" . $v_products_model_barcode . "'";
                                    $query = $updatequery;
                                }
                                else if (empty($v_products_quantity)){

                                    $updatequery = "UPDATE `products` SET `kiev_stock`='" . $v_products_quantity_kiev . "' WHERE `products_barcode`='" . $v_products_model_barcode . "'";
                                    $query = $updatequery;
                                }
                                else {
                                    $updatequery = "UPDATE `products` SET `products_quantity`='" . $v_products_quantity . "',`kiev_stock`='" . $v_products_quantity_kiev . "' WHERE `products_barcode`='" . $v_products_model_barcode . "'";
                                    $query = $updatequery;
                                }
                            }

                            else if (empty($v_products_quantity_kiev)) {
                                if (empty($v_products_quantity)){
                                    $updatequery = "UPDATE `products` SET `products_price`='" . $v_products_price . "' WHERE `products_barcode`='" . $v_products_model_barcode . "'";
                                    $query = $updatequery;
                                }
                                else{
                                    $updatequery = "UPDATE `products` SET `products_quantity`='" . $v_products_quantity . "',`products_price`='" . $v_products_price . "' WHERE `products_barcode`='" . $v_products_model_barcode . "'";
                                    $query = $updatequery;
                                }
                            }

                            else if (empty($v_products_quantity)) {
                                $updatequery = "UPDATE `products` SET `kiev_stock`='" . $v_products_quantity_kiev . "',`products_price`='" . $v_products_price . "' WHERE `products_barcode`='" . $v_products_model_barcode . "'";
                                $query = $updatequery;
                            }

                            else {
                                $updatequery = "UPDATE `products` SET `products_price`='" . $v_products_price . "',`products_quantity`='" . $v_products_quantity . "',`kiev_stock`='" . $v_products_quantity_kiev . "' WHERE `products_barcode`='" . $v_products_model_barcode . "'";
                                $query = $updatequery;
                            }
                            //Кінець
                        } else {
                           // products_date_added=" . $v_date_added . ",
                            ...
                            $query = "UPDATE " . TABLE_PRODUCTS . "
                              SET
                                $v_products_price_status_upd
                                products_price_2='$v_products_price_dealer',
                                ...
                                kiev_stock = $v_products_quantity_kiev,
                                our_stock = $v_products_quantity_our,
                                mankovka_stock = $v_products_quantity_mank,
