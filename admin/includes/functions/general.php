...
*/
  ...
  function tep_make_cat_list($parent_cat=0){
    ...
    while ($row = tep_db_fetch_array($result)) {
      ...
      if($row['parent_id']==$parent_cat) $table2[$parent_cat][] = $row['categories_id'];

    }
    $table3 = tep_rec_cats($table,$table2[$parent_cat]);

    if(is_array($table3)) $table3 = array_merge($table3, $table2[$parent_cat]);
    else $table3 = $table2[$parent_cat];
  ...
  function tep_rec_cats($table,$table2) {
   if(is_array($table2)) {
    ...
    if(!empty($table3)) {
      ...
      if(is_array($table4)) $table3 = array_merge($table3, $table4);
    }
    return $table3;
   }
  }
...
function tep_info_articles_image($image, $alt, $width = '', $height = '') {
    if (tep_not_null($image) && (file_exists(DIR_FS_CATALOG_ARTICLES_IMAGES . $image)) ) {
        $image = tep_image(DIR_WS_CATALOG_ARTICLES_IMAGES . $image, $alt, $width, $height);
    } else {
        $image = TEXT_IMAGE_NONEXISTENT;
    }
  ...
  function tep_address_format($address_format_id, $address, $html, $boln, $eoln) {
    if(is_null($address_format_id) or $address_format_id == '0') $address_format_id = 1;
...
// Output the tax percentage with optional padded decimals
  ...
  //  $headers .= "From: =?cp-1251?B?".base64_encode($from_email_name)."?= <".$from_email_address.">\r\n";
  //  mail($to_email_address, '=?cp-1251?B?'.base64_encode($email_subject).'?=', $email_text,$headers);
/*    $email_text = iconv('windows-1251', 'UTF-8', $email_text);
    $from_email_name = iconv('windows-1251', 'UTF-8', $from_email_name);
    $email_subject = iconv('windows-1251', 'UTF-8', $email_subject);*/

    $headers .= 'Content-type: text/html; charset=utf-8' . "\r\n";
    $headers .= "From: =?utf-8?B?".base64_encode($from_email_name)."?= <".$from_email_address.">\r\n";
    mail($to_email_address, '=?utf-8?B?'.base64_encode($email_subject).'?=', $email_text,$headers);
  }
...
function tep_reset_sef_cache($action)
{
    if($action === 'true')
    {
        tep_db_query("DELETE FROM `cache` WHERE `cache_name` LIKE 'sef.%'");
        tep_db_query("UPDATE `configuration` SET `configuration_value` = 'false' WHERE `configuration_key` = 'RESET_SEF_CACHE'");
    }
    # The return value is used to set the value upon viewing
    # It's NOT returining a false to indicate failure!!
    return 'false';
