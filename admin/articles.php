...
*/
  ...
  if ($action === 'insert_topic' || $action === 'update_topic' || $action === 'delete_topic_confirm' || $action === 'delete_article_confirm' || $action === 'insert_article' || $action === 'update_article' || $action === 'copy_to_confirm') {
    tep_db_query("DELETE FROM cache WHERE `cache_name` LIKE 'sef.articles.%'");
  }
        ...
        if (isset($_POST['edit_x']) || isset($_POST['edit_y'])) {
          ...
          for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
            ...
            $sql_data_array = array('articles_name' => tep_db_prepare_input($_POST['articles_name'][$language_id]),
                                    ...
                                    'articles_image' => tep_db_prepare_input($_POST['articles_image'][$language_id]),
      ...
      case 'copy_to_confirm':
        if (isset($_POST['articles_id']) && isset($_POST['topics_id'])) {
          ...
          if ($_POST['copy_as'] == 'link') {
            ...
            $article_query = tep_db_query("select articles_date_available from " . TABLE_ARTICLES . " where articles_id = '" . (int)$articles_id . "'");
            $article = tep_db_fetch_array($article_query);

            tep_db_query("insert into " . TABLE_ARTICLES . " (articles_date_added, articles_date_available, articles_status) values (now(), '" . tep_db_input($article['articles_date_available']) . "', '0')");
            $dup_articles_id = tep_db_insert_id();

            $description_query = tep_db_query("select language_id, articles_name, articles_description, articles_url, articles_image, articles_head_title_tag, articles_head_desc_tag, articles_head_keywords_tag from " . TABLE_ARTICLES_DESCRIPTION . " where articles_id = '" . (int)$articles_id . "'");
            while ($description = tep_db_fetch_array($description_query)) {
              tep_db_query("insert into " . TABLE_ARTICLES_DESCRIPTION . " (articles_id, language_id, articles_name, articles_description, articles_url, articles_image, articles_head_title_tag, articles_head_desc_tag, articles_head_keywords_tag, articles_viewed) values ('" . (int)$dup_articles_id . "', '" . (int)$description['language_id'] . "', '" . tep_db_input($description['articles_name']) . "', '" . tep_db_input($description['articles_description']) . "', '" . tep_db_input($description['articles_url']) . "',  '" . tep_db_input($description['articles_image']) . "', '" . tep_db_input($description['articles_head_title_tag']) . "', '" . tep_db_input($description['articles_head_desc_tag']) . "', '" . tep_db_input($description['articles_head_keywords_tag']) . "', '0')");
            }
...
<?php
          echo tep_draw_textarea_field('topics_description[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (($topics_description[$languages[$i]['id']]) ? stripslashes($topics_description[$languages[$i]['id']]) : tep_get_topic_description($tInfo->topics_id, $languages[$i]['id'])), 'class="ckeditor"'); 
?>
<?php
   //       echo tep_draw_textarea_field('newsdesk_article_shorttext_' . $languages[$i]['id'] . '', 'soft', '80', '12', (isset($newsdesk_article_shorttext[$languages[$i]['id']]) ? stripslashes($newsdesk_article_shorttext[$languages[$i]['id']]) : newsdesk_get_newsdesk_article_shorttext($pInfo->newsdesk_id, $languages[$i]['id'])), 'class="ckeditor"');
?>
    <script type="text/javascript">
      var editor = CKEDITOR.replace( 'topics_description[<?php echo $languages[$i][id]; ?>]');
...
<?php
    ...
    if (isset($_GET['aID']) && empty($_POST)) {
      $article_query = tep_db_query("select ad.articles_name, ad.articles_description, ad.articles_url, ad.articles_image, ad.articles_head_title_tag, ad.articles_head_desc_tag, ad.articles_head_keywords_tag, a.articles_id, a.articles_date_added, a.articles_last_modified, date_format(a.articles_date_available, '%Y-%m-%d') as articles_date_available, a.articles_status from " . TABLE_ARTICLES . " a, " . TABLE_ARTICLES_DESCRIPTION . " ad where a.articles_id = '" . (int)$_GET['aID'] . "' and a.articles_id = ad.articles_id and ad.language_id = '" . (int)$languages_id . "'");
      $article = tep_db_fetch_array($article_query);
...
?>
          <tr bgcolor="#ffffcc">
            ...
            <td><table border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="main" valign="top"><?php echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']); ?>&nbsp;</td>
                <td class="main"><?php echo tep_draw_textarea_field('articles_head_desc_tag[' . $languages[$i]['id'] . ']', 'soft', '70', '5', (isset($articles_head_desc_tag[$languages[$i]['id']]) ? $articles_head_desc_tag[$languages[$i]['id']] : tep_get_articles_head_desc_tag($aInfo->articles_id, $languages[$i]['id'])),'class="notinymce"'); ?></td>
              </tr>
            </table></td>
          </tr>
<?php
...
?>
          <tr bgcolor="#ffffcc">
            ...
            <td><table border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="main" valign="top"><?php echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']); ?>&nbsp;</td>
                <td class="main"><?php echo tep_draw_textarea_field('articles_head_keywords_tag[' . $languages[$i]['id'] . ']', 'soft', '70', '5', (isset($articles_head_keywords_tag[$languages[$i]['id']]) ? $articles_head_keywords_tag[$languages[$i]['id']] : tep_get_articles_head_keywords_tag($aInfo->articles_id, $languages[$i]['id'])),'class="notinymce"'); ?></td>
              </tr>
            </table></td>
          </tr>
<?php
...
<?php //echo tep_draw_fckeditor('articles_description[' . $languages[$i]['id'] . ']','800','500',(isset($articles_description[$languages[$i]['id']]) ? stripslashes($articles_description[$languages[$i]['id']]) : tep_get_articles_description($aInfo->articles_id, $languages[$i]['id'])));
?>
...
?>
<script type="text/javascript">
      var editor = CKEDITOR.replace( 'articles_description[<?php echo $languages[$i]['id']; ?>]');
      CKFinder.setupCKEditor( editor, '../includes/ckfinder/' ) ;
    </script>
...
<?php
    ...
    //if ($_POST['delete_image'] == 'yes') { unlink(DIR_FS_CATALOG_IMAGES . $aInfo->articles_image2); }
?>

<!-- Картинка статьи\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ -->
<tr>
  <td class="main">Картинка</td>
  <td valign="top" class="main"><?php echo articles_draw_file_field('articles_image', 'size="30"').'<br>
  <input type="checkbox" name="delete_image" value="yes">Удалить картинку' . tep_draw_separator('pixel_trans.gif', '1', '5');?></td> 
</tr>

          <?php 
          echo tep_draw_hidden_field('articles_image2', $aInfo->articles_image); 
          echo (($aInfo->articles_image) ? tep_image(DIR_WS_CATALOG_IMAGES . $aInfo->articles_image,'',200,200):TEXT_IMAGE_NONEXISTENT);
         ?> </td></tr>
<!-- Картинка статьи\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\ -->
...
<?php
   ...
   if ( ($articles_image != 'none') && ($articles_image != '') ) {
         $articles_image = tep_get_uploaded_file('articles_image');
        $image_directory = tep_get_local_path(DIR_FS_CATALOG_IMAGES);
      }
   if (is_uploaded_file($articles_image['tmp_name'])) {
        tep_copy_uploaded_file($articles_image, $image_directory);
        $articles_image_name = $articles_image['name'];
  } else {
        $articles_image_name = $_POST['products_previous_image'];
      }       
      
      
    } else {
      $article_query = tep_db_query("select a.articles_id, ad.language_id, ad.articles_name, ad.articles_description, ad.articles_url, ad.articles_image, ad.articles_head_title_tag, ad.articles_head_desc_tag, ad.articles_head_keywords_tag, a.articles_date_added, a.articles_last_modified, a.articles_date_available, a.articles_status  from " . TABLE_ARTICLES . " a, " . TABLE_ARTICLES_DESCRIPTION . " ad where a.articles_id = ad.articles_id and a.articles_id = '" . (int)$_GET['aID'] . "'");
      $article = tep_db_fetch_array($article_query);
    ...
    for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
      if (isset($_GET['read']) && ($_GET['read'] == 'only')) {
        ...
        $aInfo->articles_name2 = tep_db_prepare_input($articles_name[$languages[$i]['id']]);
        ...
        $articles_image_name = $pInfo->articles_image;    
...
?>
      ...
      <tr>
        <td class="main"><?php echo sprintf(TEXT_ARTICLE_MORE_INFORMATION, $aInfo->articles_image); ?></td>
      </tr>
<?php
      }
?>
...
//Удаление и перезапись картинки
         
        if($_FILES['articles_image']['name'] == '' && $_POST['delete_image'] == 'yes'){ echo tep_draw_hidden_field('articles_image[' . $languages[$i]['id'] . ']', ''); unlink(DIR_FS_CATALOG_IMAGES . $aInfo->articles_image2); }
        if($_FILES['articles_image']['name'] == '' && $_POST['delete_image'] == ''){ echo tep_draw_hidden_field('articles_image[' . $languages[$i]['id'] . ']', $aInfo->articles_image2); } 
        if($_FILES['articles_image']['name'] != '' && $_POST['delete_image'] == 'yes'){ echo tep_draw_hidden_field('articles_image[' . $languages[$i]['id'] . ']', ''); unlink(DIR_FS_CATALOG_IMAGES . $aInfo->articles_image2);} 
        if($_FILES['articles_image']['name'] != '' && $_POST['delete_image'] == ''){ echo tep_draw_hidden_field('articles_image[' . $languages[$i]['id'] . ']', $_FILES['articles_image']['name']);}     
