...
//      $group=$xml->get_content($product."/group[1]");
      ...
      $products_name=unhtmlentities($xml->get_content($product."/products_name[1]"));
      $products_quantity=$xml->get_content($product."/products_quantity[1]");
      $products_price=$xml->get_content($product."/products_price[1]");
      $products_date_available=$xml->get_content($product."/products_date_available[1]");
      $products_status=$xml->get_content($product."/products_status[1]");
      ...
      if (tep_db_num_rows($products_query)) {
        ...
        if ($row['products_price']!=$products_price) {
          tep_db_perform(TABLE_PRODUCTS, array('products_last_modified' => 'now()', 'products_price' => $products_price, 'products_status' => $products_status, 'products_quantity' => $products_quantity, 'products_date_available' => $products_date_available), 'update', 'products_id=\''.$products_id.'\'');
          tep_db_perform(TABLE_PRODUCTS_DESCRIPTION, array('products_name' => $products_name), 'update', 'products_id=\''.$products_id.'\' and language_id=\''.$languages_id.'\'');
          $count_upd++;
        }
      } else {
        tep_db_perform(TABLE_PRODUCTS, array('products_id' => $products_id, 'products_last_modified' => 'now()', 'products_price' => $products_price, 'products_status' => 0, 'products_quantity' => $products_quantity, 'products_date_available' => $products_date_available));
        tep_db_perform(TABLE_PRODUCTS_DESCRIPTION, array('products_id' => $products_id, 'products_name' => $products_name, 'language_id' => $languages_id));
        tep_db_perform(TABLE_PRODUCTS_TO_CATEGORIES, array('products_id' => $products_id, 'categories_id' => 0));
        $count_add++;
      }
      $count++;
    }
    $messageStack->add_session('Updated '.$count_upd, 'success');
    $messageStack->add_session('Not changed '.($count-$count_upd), 'success');
    $messageStack->add_session('Added '.$count_add, 'success');
  } else {
    $messageStack->add_session('File not uploaded', 'error');
  }
...
if ($_POST['action']=='export') {
  ...
  if(get_categories_ids($_POST['categories_id'])!='')
	  $categories_ids=array_merge($categories_ids, get_categories_ids($_POST['categories_id']));

  $categories_ids = array_filter($categories_ids);

  $products=tep_db_query("select * from ".TABLE_PRODUCTS." left join ".TABLE_PRODUCTS_DESCRIPTION." using(products_id) left join ".TABLE_PRODUCTS_TO_CATEGORIES." using(products_id) where language_id='".$languages_id."' and categories_id in (".implode($categories_ids, ', ').")");
...
<?php
  switch ($_POST['arc']) {
    ...
    case 'zip': $ext='.xml.zip';
                $zipfile = new zipfile();
                $zipfile->addFile($xml_file, DB_DATABASE.'.xml');
  ...
  header("Content-Disposition: attachment; filename=".DB_DATABASE.$ext);
...
<!-- body_text //-->
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
      ...
      <tr>
        ...
        <td class="main">
          <?php echo tep_draw_form('xml_import', basename($_SERVER['PHP_SELF']), '', 'post', 'enctype="multipart/form-data"') ."\n". tep_draw_file_field('xml_file') ."\n"; ?>
          <input type="hidden" name="action" value="import">
          <br>
          Max file size <b><?php echo ini_get('upload_max_filesize'); ?></b>
      ...
      <tr>
        <td class="main" colspan="2">
          <ul>
            <li>This module allows to update the following fields: products_name (according to the current language, current session language is <b><?php echo $language; ?></b>), products_quantity, products_price (according to the default currency), products_date_available, products_status
            <li>To edit a XML-file it is possible with the help of any XML-editor, for example <a href="http://xmlmarker.com" target="_blank" style="color: #0000FF;">XML Marker</a>.
          </ul>
        </td>
      </tr>
    </table></td>
