    ...
    if (tep_class_exists($class)) {
      ...
      if ((!isset($_GET['module']) || (isset($_GET['module']) && ($_GET['module'] == $class))) && !isset($mInfo)) {
        ...
        for ($j=0, $k=sizeof($module_keys); $j<$k; $j++) {
          $key_value_query = tep_db_query("select configuration_title, configuration_key, configuration_value, configuration_description, use_function, set_function, configuration_title_eng, configuration_description_eng from " . TABLE_CONFIGURATION . " where configuration_key = '" . $module_keys[$j] . "'");
		  ...
		  $keys_extra[$module_keys[$j]]['title_eng'] = $key_value['configuration_title_eng'];
		  $keys_extra[$module_keys[$j]]['key_table'] = $key_value['configuration_key'];
		  $keys_extra[$module_keys[$j]]['description_eng'] = $key_value['configuration_description_eng'];
  ...
  switch ($action) {
    case 'edit':
      ...
      while (list($key, $value) = each($mInfo->keys)) {
		...
		{
			$keys .= '<b>' . $value['title_eng'] .'</b><br>';
			$keys .= $value['description_eng'] .  '<br>';
		} 	
