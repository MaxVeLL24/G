...
*/
  ...
  switch ($HTTP_GET_VARS['action']) {
		case 'deleterecords':
      unlink(DIR_FS_CATALOG.'includes/slow_queries/slow_query_log.txt');
      tep_redirect(tep_href_link(FILENAME_MYSQL_PERFORMANCE));
			break;
		case 'deleteconfirm':
      cutline(DIR_FS_CATALOG.'includes/slow_queries/slow_query_log.txt', $HTTP_GET_VARS['qID']);
...
<?php
  ...
  if ($Lines){
	  ...
	  if ($performance_numrows> $r_maxrows){ // if there are more records than will fit on one page
	    $performance_start_row=($HTTP_GET_VARS['page']* $r_maxrows)+1;
	    $performance_end_row= $performance_start+ $r_maxrows;
            ...
            <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr>
                <td class="smallText" valign="top"><?php echo sprintf(TEXT_DISPLAY_NUMBER_OF_QUERIES,  ($Lines)? $performance_start_row+1 : $performance_start_row,$performance_end_row,$performance_numrows); ?></td>
                 <td class="smallText" align="right"><?php echo display_links($performance_numrows, $r_maxrows, MAX_DISPLAY_PAGE_LINKS, $HTTP_GET_VARS['page']); ?></td>
              </tr>
            </table></td>
          </tr>
          <tr>
