...
function amSendRequest(requestString,functionName, refresh, target) {
    ...
    setTimeout(function () {
        $('#attributeManager select:not(#template_drop):not([name=prefix]):not(#optionSortDropDown)').chosen({width: "200px"});
    },300);
	return false;
}
...
function amUpdate(optionId, optionValueId, optionSender) {
  ...
  pa_qty=getDropDownValue('pa_qty_'+optionValueId);
	...
	amSendRequest('amAction=update&option_id='+optionId+'&option_value_id='+optionValueId+'&price='+getDropDownValue('price_'+optionValueId)+'&prefix='+getDropDownValue('prefix_'+optionValueId)+'&pa_qty='+getDropDownValue('pa_qty_'+optionValueId)+'&sortOrder='+getDropDownValue('sortOrder_'+optionValueId)+'&weight='+getDropDownValue('weight_'+optionValueId)+'&weight_prefix='+getDropDownValue('weight_prefix_'+optionValueId),'',false);
...
//  alert(pa_qty);
	...
	if(0 == option || 0 == optionValue)
		return false;
	amSendRequest('amAction=addAttributeToProduct&option_id='+option+'&option_value_id='+optionValue+'&prefix='+pricePrefix+'&pa_qty='+pa_qty+'&price='+price+'&sortOrder='+sortOrder+'&weight_prefix='+weightPrefix+'&weight='+weight);
	return false;
}
