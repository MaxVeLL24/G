...
<?php
    ...
    while ($featured = tep_db_fetch_array($featured_query)) {
      if ( ((!$_GET['sID']) || ($_GET['sID'] == $featured['featured_id'])) && (!$sInfo) ) {
        ...
        $products_query = tep_db_query("select products_images from " . TABLE_PRODUCTS . " where products_id = '" . $featured['products_id'] . "'");
  ...
  switch ($_GET['action']) {
    ...
    default:
      if (is_object($sInfo)) {
        ...
        $img = explode(';', $sInfo->products_images);
        $contents[] = array('align' => 'center', 'text' => '<br>' . tep_info_image($img[0], $sInfo->products_name, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT));
