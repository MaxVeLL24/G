...
// Закрыть ссылки фильтров в nofollow на страницах брендов и результатов поиска
$nofollow_links = isset($_GET['manufacturers_id']) || isset($_GET['keywords']);
...
<div class="box-selected-filters common-styled-block">
    ...
    <a
        class="clear-selected-filters"
        href="<?php echo tep_href_link(FILENAME_DEFAULT, manageGetParams($options_ids_in_query, null)); ?>"
        <?php if($nofollow_links) : ?>rel="nofollow"<?php endif; ?>
        ><?php echo TEXT_CLEAR_SELECTED_FILTERS; ?></a>
    <dl>
        ...
        <dd>
            <ul>
                ...
                // Добавление или исключение текущего значения опции в/из запроса
                $tmp = array_intersect($options_to_options_values[$option_id], $options_values_ids_in_query);
                if(in_array($option_value_id, $options_values_ids_in_query))
                {
                    $tmp = array_diff($tmp, array($option_value_id));
                }
                if($tmp)
                {
                    $tmp = manageGetParams(array('page'), array($option_id => implode('-', $tmp)));
                }
                else
                {
                    $tmp = manageGetParams(array('page', $option_id), null);
                }
                
                ?>
                <li>
                    <a
                        href="<?php echo tep_href_link(FILENAME_DEFAULT, $tmp); ?>"
                        <?php if($nofollow_links) : ?>rel="nofollow"<?php endif; ?>
                        ><?php echo tep_escape($options_values[$option_value_id]); ?></a>
                </li>
                <?php endif; ?>
                <?php endforeach; ?>
            </ul>
...
<div class="box-filter common-styled-block">
    <?php /* Не показывать фильтр по цене, если минимальная и максимальная возможные цены совпадают */ ?>
    ...
    <div class="options-group">
        ...
        <div class="options">
            <?php if(!in_array($option_id, $options_ids_in_query)) : ?>
            ...
            <a
                class="option"
                href="<?php echo tep_href_link(FILENAME_DEFAULT, manageGetParams(array('page', $option_id), null)) ?>"
                <?php if($nofollow_links) : ?>rel="nofollow"<?php endif; ?>
                >
                <span class="checkbox-label"></span>
                <span class="control-label">
            ...
            <?php endif; ?>
            <?php foreach($options_to_options_values[$option_id] as $option_value_id) : ?>
            <?php if($options_values_availability[$option_value_id]) : ?>
            <?php
            ...
            {
                $tmp = array_merge($tmp, array($option_value_id));
            }
            if($tmp)
            {
                $tmp = manageGetParams(array('page'), array($option_id => implode('-', $tmp)));
            ...
            ?>
            <a
                class="option<?php if(in_array($option_value_id, $options_values_ids_in_query)) : ?> checked<?php endif; ?>"
                href="<?php echo tep_href_link(FILENAME_DEFAULT, $tmp); ?>"
                <?php if($nofollow_links) : ?>rel="nofollow"<?php endif; ?>
                >
                <span class="checkbox-label"></span>
                <span class="control-label">
                    ...
                    <?php if(!empty($options_values_products_count[$option_value_id])) : ?>
                    <span class="count"><?php echo $options_values_products_count[$option_value_id]; ?></span>
                    <?php endif; ?>
                </span>
            </a>
            <?php endif; ?>
            ...
            ?>
            <div class="option<?php if(in_array($option_value_id, $options_values_ids_in_query)) : ?> checked<?php endif; ?> disabled">
                <span class="checkbox-label"></span>
                <span class="control-label">
                    <span class="label"><?php echo tep_escape($options_values[$option_value_id]); ?></span>
                    <?php if(!empty($options_values_products_count[$option_value_id])) : ?>
