	...
	function GenerateProductSitemap(){
		...
		if ( $products_query = $this->DB->Query($sql) ){
			...
			while( $result = $this->DB->FetchArray($products_query) ){
                ...
                //$priority = $ratio < .1 ? .8 : number_format($ratio, 1, '.', '');
                $priority=0.8;

                $_SESSION['languages_id']='4';
                $locationUA = $this->hrefLink(''.FILENAME_PRODUCT_INFO,'products_id=' . $result['pID'], 'NONSSL', false);

// SiteProducts -> here
				$container[] = array('loc' => htmlspecialchars(utf8_encode($location)),
                    'loc_ua' => htmlspecialchars(utf8_encode($locationUA)),
	...
	function GenerateCategorySitemap(){
		...
		if ( $categories_query = $this->DB->Query($sql) ){
			...
			while( $result = $this->DB->FetchArray($categories_query) ){
				...
				$priority = 0.9;
                $_SESSION['languages_id']='4';
                $locationUA = $this->hrefLink(FILENAME_DEFAULT, 'cPath=' . $this->GetFullcPath($result['cID']), 'NONSSL', false);
