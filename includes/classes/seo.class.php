...
//ojp FILENAME_LINKS
        ...
        if (defined('FILENAME_FAQDESK_INFO'))
            $seo_pages[] = FILENAME_FAQDESK_INFO;
        if (defined('FILENAME_FAQDESK_INDEX'))
            $seo_pages[] = FILENAME_FAQDESK_INDEX;
        if (defined('FILENAME_FAQDESK_REVIEWS_INFO'))
            $seo_pages[] = FILENAME_FAQDESK_REVIEWS_INFO;
        if (defined('FILENAME_FAQDESK_REVIEWS_ARTICLE'))
            $seo_pages[] = FILENAME_FAQDESK_REVIEWS_ARTICLE;
        ...
        if (defined('FILENAME_NEWSDESK_INFO'))
            $seo_pages[] = FILENAME_NEWSDESK_INFO;
        if (defined('FILENAME_NEWSDESK_INDEX'))
            $seo_pages[] = FILENAME_NEWSDESK_INDEX;
        if (defined('FILENAME_NEWSDESK_REVIEWS_INFO'))
            $seo_pages[] = FILENAME_NEWSDESK_REVIEWS_INFO;
        if (defined('FILENAME_NEWSDESK_REVIEWS_ARTICLE'))
            $seo_pages[] = FILENAME_NEWSDESK_REVIEWS_ARTICLE;
        if (defined('FILENAME_LINKS'))
            $seo_pages[] = FILENAME_LINKS;
...
//ojp generate_link_cache
        if ($this->attributes['USE_SEO_CACHE_GLOBAL'] == 'true') {
            ...
            if ($this->attributes['USE_SEO_CACHE_LINK_PAGES'] == 'true' && defined('TABLE_LINK_CATEGORIES'))
...
# end constructor
    ...
    function href_link($page = '', $parameters = '', $connection = 'NONSSL', $add_session_id = false) {
        ...
        if ($_link) {
            if ($_link['path']) {
                $path = explode('/', $_link['path']);
                if ($path) {
                    $last_item = explode('.', array_pop($path));
                    foreach ($last_item as $key => $value) {
                        $last_item[$key] = tep_make_uri_friendly_string($value);
                    }
                    $last_item = implode('.', $last_item);
                    foreach ($path as $key => $value) {
                        $path[$key] = tep_make_uri_friendly_string($value);
                    }
                    $path[] = $last_item;
                    $_link['path'] = implode('/', $path);
                }
            }
            $link = '';
            if ($_link['scheme']) {
                ...
                case 'newsdesk_id':
                    switch (true) {
                        case ($page == FILENAME_NEWSDESK_INFO):
                            ...
                            $this->ValidateName($url, "n", $p2[1], $connection, $separator);
                            break;
                        case ($page == FILENAME_NEWSDESK_REVIEWS_INFO):
                            ...
                            $this->ValidateName($url, "nri", $p2[1], $connection, $separator);
                            break;
                        case ($page == FILENAME_NEWSDESK_REVIEWS_ARTICLE):
                            ...
                            $this->ValidateName($url, "nra", $p2[1], $connection, $separator);
                            break;
                        default:
                            $container[$p2[0]] = $p2[1];
                            break;
                ...
                case 'newsPath':
                    switch (true) {
                        case ($page == FILENAME_NEWSDESK_INDEX):
                            ...
                            $this->ValidateName($url, "nc", $p2[1], $connection, $separator);
                            break;
                        default:
                            $container[$p2[0]] = $p2[1];
                            break;
    ...
    function get_product_name($pID) {
        switch (true) {
            ...
            default:
                ...
                $this->cache['PRODUCTS'][$pID] = $result['pName'];
                $this->performance['QUERIES']['PRODUCTS'][] = $sql;
                $return = $result['pName'];
                break;
        } # end switch
        return tep_make_uri_friendly_string($return);
    }
...
# end function
    ...
    function Validatename($url, $type, $realID, $connection, $separator) {
        ...
        $end_part = explode($type . '-', $parts[count($parts) - 1]);
        
        $end_part = preg_replace('/-/', '', $end_part);
        $end_part = implode($type . '-', $end_part);
        $new_type = explode('-', $end_part); //?? ???
        
        if ($new_type[0] == $type) {  //make sure it is the correct type for this link
            if (($pos = strpos($new_type[1], ".html")) !== FALSE)
                $id = substr($new_type[1], 0, $pos);       //strip .html
            $id = urldecode($id);
            $id = preg_replace('/[^0-9,_]/i', '', $id);
        ...
        switch (true) {
            ...
            default:
                ...
                $sql = "SELECT articles_name as aName
                        ...
                        LIMIT 1";
                $result = $this->DB->FetchArray($this->DB->Query($sql));
                $this->cache['ARTICLES'][$aID] = $result['aName'];
        ...
        switch (true) {
            ...
            default:
                ...
                $sql = "SELECT authors_name as auName
                        ...
                        LIMIT 1";
                $result = $this->DB->FetchArray($this->DB->Query($sql));
                $this->cache['AUTHORS'][$auID] = $result['auName'];
                $this->performance['QUERIES']['AUTHORS'][] = $sql;
                $return = $result['auName'];
                break;
        } # end switch
        return tep_make_uri_friendly_string($return);
    }
        ...
        switch (true) {
            ...
            default:
                ...
                $sql = "SELECT topics_name as tName
                        ...
                        LIMIT 1";
                $result = $this->DB->FetchArray($this->DB->Query($sql));
                $this->cache['ARTICLES'][$aID] = $result['tName'];
        ...
        switch (true) {
            ...
            default:
                ...
                $sql = "SELECT link_categories_name as lName
                        ...
                        LIMIT 1";
                $result = $this->DB->FetchArray($this->DB->Query($sql));
                $this->cache['ARTICLES'][$aID] = $result['lName'];
                $this->performance['QUERIES']['TOPICS'][] = $sql;
                $return = $result['lName'];
                break;
        } # end switch
    ...
    function get_information_name($iID) {
        switch (true) {
            ...
            default:
             ...
             LIMIT 1";
                $result = $this->DB->FetchArray($this->DB->Query($sql));
                $this->cache['INFO'][$iID] = $result['iName'];
        ...
        switch (true) {
            ...
            default:
                ...
                $sql = "SELECT faqdesk_question as fName
                                                ...
                                                LIMIT 1 ";
                $result = $this->DB->FetchArray($this->DB->Query($sql));
                $this->cache['FAQDESK'][$fID] = $result['fName'];
        ...
        switch (true) {
            ...
            default:
                ...
                $sql = "SELECT categories_name as fcName
                                                ...
                                                LIMIT 1 ";
                $result = $this->DB->FetchArray($this->DB->Query($sql));
                $this->cache['FAQDESK_CATEGORIES'][$fcID] = $result['fcName'];
                $this->performance['QUERIES']['FAQDESK_CATEGORIES'][] = $sql;
                $return = $result['fcName'];
                break;
        } # end switch
        return tep_make_uri_friendly_string($return);
    }
...
# end function
    ...
    function get_pages_name($pmID) {
        switch (true) {
            ...
            default:
                $this->performance['NUMBER_QUERIES'] ++;
                $sql = "SELECT pages_name as pmName
                        FROM " . TABLE_PAGES_DESCRIPTION . "
                        WHERE pages_id='" . (int) $pmID . "'
                        AND language_id='" . (int) $this->languages_id . "'
                        LIMIT 1";
                $result = $this->DB->FetchArray($this->DB->Query($sql));
                $this->cache['PAGES'][$pmID] = $result['pmName'];
                $this->performance['QUERIES']['PAGES'][] = $sql;
                $return = $result['pmName'];
                break;
        } # end switch
        return tep_make_uri_friendly_string($return);
    }
        ...
        switch (true) {
            ...
            default:
                ...
                $sql = "SELECT newsdesk_article_name as nName
                                                ...
                                                LIMIT 1 ";
                $result = $this->DB->FetchArray($this->DB->Query($sql));
                $this->cache['NEWSDESK'][$nID] = $result['nName'];
        ...
        switch (true) {
            ...
            default:
                ...
                $sql = "SELECT categories_name as ncName
                                                ...
                                                LIMIT 1 ";
                $result = $this->DB->FetchArray($this->DB->Query($sql));
                $this->cache['NEWSDESK_CATEGORIES'][$ncID] = $result['ncName'];
                $this->performance['QUERIES']['NEWSDESK_CATEGORIES'][] = $sql;
                $return = $result['ncName'];
                break;
        } # end switch
        return tep_make_uri_friendly_string($return);
    }
...
# end function
    ...
    function generate_products_cache() {
        ...
        if (!$is_cached || $is_expired) {
            $sql = "SELECT p.products_id as id, pd.products_name, pd.products_url FROM " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd WHERE p.products_id=pd.products_id and p.products_status='1' and pd.language_id='" . (int) $this->languages_id . "'";
            $product_query = $this->DB->Query($sql);
            $prod_cache = '';
            while ($product = $this->DB->FetchArray($product_query)) {
                if (!$product['products_url']) {
                    $product['products_url'] = $product['products_name'];
                }
                $product['products_url'] = tep_make_uri_friendly_string($product['products_url']);
                $define = 'define(\'PRODUCT_NAME_' . $product['products_id'] . '\', \'' . $this->strip($product['products_url']) . '\');';
                $prod_cache .= $define . "\n";
                eval("$define");
            }
...
# end function
    ...
    function generate_categories_cache() {
        ...
        if (!$is_cached || $is_expired) { // it's not cached so create it
            switch (true) {
                case ($this->attributes['SEO_ADD_CAT_PARENT'] == 'true'):
                    $sql = "SELECT c.categories_id as id, c.parent_id, cd.categories_url as cName, cd2.categories_url as pName
                            FROM " . TABLE_CATEGORIES . " c
                            JOIN " . TABLE_CATEGORIES_DESCRIPTION . " cd
                            ON c.categories_id = cd.categories_id
                            LEFT JOIN " . TABLE_CATEGORIES_DESCRIPTION . " cd2
                    ...
                    break;
                default:
                    $sql = "SELECT categories_id as id, categories_url as cName
                            FROM " . TABLE_CATEGORIES_DESCRIPTION . "
                            WHERE language_id='" . (int) $this->languages_id . "'";
                    break;
            } # end switch
