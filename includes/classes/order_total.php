...
class order_total
{
    ...
    var $modules;
    public static $order_total_classes_objects = array();
    
    /**
     * Возвращает объект указанного класса
     * @param string $order_total_class_name
     * @return object|null
     */
    public static function getInstanceOf($order_total_class_name)
    {
        if(!class_exists($order_total_class_name))
        {
            if(is_file(DIR_WS_MODULES . 'order_total/' . $order_total_class_name . '.php'))
            {
                include DIR_WS_MODULES . 'order_total/' . $order_total_class_name . '.php';
                if(!class_exists($order_total_class_name))
                {
                    return null;
                }
            ...
            if(is_file(DIR_WS_LANGUAGES . $_SESSION['language'] . '/modules/order_total/' . $order_total_class_name . '.php'))
            {
                include DIR_WS_LANGUAGES . $_SESSION['language'] . '/modules/order_total/' . $order_total_class_name . '.php';
            }
        }
        if(empty(self::$order_total_classes_objects[$order_total_class_name]))
        {
            self::$order_total_classes_objects[$order_total_class_name] = new $order_total_class_name();
        }
        return self::$order_total_classes_objects[$order_total_class_name];
    }
        ...
        if(is_array($this->modules))
        {
            ...
            while(list(, $value) = each($this->modules))
            {
                ...
                if(($_module = self::getInstanceOf($class)) && $_module->enabled)
                {
                    ...
                    for($i = 0, $n = sizeof($_module->output); $i < $n; $i++)
                    {
                        if(tep_not_null($_module->output[$i]['title']) && tep_not_null($_module->output[$i]['text']))
                        {
                            $order_total_array[] = array('code' => $_module->code,
                                'title' => $_module->output[$i]['title'],
                                'text' => $_module->output[$i]['text'],
                                'value' => $_module->output[$i]['value'],
    ...
    function output()
    {
        ...
        if(is_array($this->modules))
        {
            ...
            while(list(, $value) = each($this->modules))
            {
                ...
                if(($_module = self::getInstanceOf($class)) && $_module->enabled)
                {
                    $size = sizeof($_module->output);
                    for($i = 0; $i < $size; $i++)
                    {
                        $output_string .= '<div class="order-total"><span class="title">' . $_module->output[$i]['title'] . '</span> <span class="value">' . $_module->output[$i]['text'] . '</span></div>';
                    }
                }
            }
        }
...
//
    function credit_selection()
    {
        ...
        if(MODULE_ORDER_TOTAL_INSTALLED)
        {
            ...
            {
                $class = substr($value, 0, strrpos($value, '.'));
                if(($_module = self::getInstanceOf($class)) && $_module->enabled && $_module->credit_class)
                {
                    $use_credit_string = $_module->use_credit_amount();
                    if($selection_string == '')
                        $selection_string = $_module->credit_selection();
                    if(($use_credit_string != '' ) || ($selection_string != ''))
                    {
                        ...
                        $output_string = ' <tr class="moduleRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" >' . "\n" .
                                ...
                                '     <td class="main"><b>' . $_module->header . '</b></td>' . $use_credit_string;
    ...
    function update_credit_account($i)
    {
        if(MODULE_ORDER_TOTAL_INSTALLED)
        {
            ...
            while(list(, $value) = each($this->modules))
            {
                ...
                if(($_module = self::getInstanceOf($class)) && $_module->enabled && $_module->credit_class)
                {
                    $_module->update_credit_account($i);
                }
            }
        }
    }
    ...
    function pre_confirmation_check()
    {
        ...
        if(MODULE_ORDER_TOTAL_INSTALLED)
        {
            ...
            while(list(, $value) = each($this->modules))
            {
                ...
                if(($_module = self::getInstanceOf($class)) && $_module->enabled && $_module->credit_class)
                {
                    $total_deductions = $total_deductions + $_module->pre_confirmation_check($order_total);
                    $order_total = $order_total - $_module->pre_confirmation_check($order_total);
                }
            }
