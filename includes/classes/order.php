...
//KIKOLEPPARD add for color groups end							  
    ...
    function cart() {
      global $customer_id, $sendto, $billto, $cart, $languages_id, $currency, $currencies, $payment,
             $cc_type, $cc_owner, $cc_number, $cc_expires, $comments;
      ...
      $tax_address_query = tep_db_query("select ab.entry_country_id, ab.entry_zone_id from " . TABLE_ADDRESS_BOOK . " ab left join " . TABLE_ZONES . " z on (ab.entry_zone_id = z.zone_id) where ab.customers_id = '" . (int)$customer_id . "' and ab.address_book_id = '" . $sendto . "'");
      ...
      $this->info = array('order_status' => DEFAULT_ORDERS_STATUS_ID,
                          ...
                          'cc_type' => (isset($cc_type) ? $cc_type : ''),
                          'cc_owner' => (isset($cc_owner) ? $cc_owner : ''),
                          'cc_number' => (isset($cc_number) ? $cc_number : ''),
                          'cc_expires' => (isset($cc_expires) ? $cc_expires : ''),
...
/* One Page Checkout - BEGIN */
  // tax total fix start
                          'shipping_tax' => empty($_SESSION['shipping']['shipping_tax_total']) ? '' : $_SESSION['shipping']['shipping_tax_total'],
  // tax total fix end
/* One Page Checkout - END */
      ...
      if (($_module = payment::getInstanceOf($payment))) {
        $this->info['payment_method'] = $_module->title;

        if ( isset($_module->order_status) && is_numeric($_module->order_status) && ($_module->order_status > 0) ) {
          $this->info['order_status'] = $_module->order_status;
        }
      }
