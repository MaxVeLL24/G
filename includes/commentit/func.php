...
if($massparam['workcapt'] == 1)
{
    if(!extension_loaded('gd'))
    {
        if(!dl('gd2.so'))
        {
            if(!dl('php_gd2.dll'))
            {
                exit($langcommentit['core_error_gd2']);
            }
        }
    }
}
...
{
    $iconv = 0;
}if($massparam['quotebb'] == 0 || $massparam['bbpanel'] == 0)
{
    $langcommentit['skin_reply_que'] = '';
}
if($massparam['loginzaglob'] == 1)
{
    $massparam['http'] == 0;
    $massparam['viewentermail'] == 0;
}
##############
/**
...
if(@$_GET['n'])
{
    commentrating($_GET['n'], $_GET['g']);
    exit();
}
if($massparam['specurl'] == '')
{
    $specurl = $_SERVER["REQUEST_URI"];
}
else
{
    eval("\$specurl=\"$massparam[specurl]\";");
} $url = '';
$dataz = date("Y-m-d H:i:s");
$url = @$_POST['url'];
$idProduct = $_GET['products_id'];
$htmlz = "";
$hostsite = "http://" . $_SERVER['SERVER_NAME'];
$hostrss = $hostsite . "/" . $wwp . "/rss.php";
if(empty($url))
    $url = @$_GET['url'];$zzzzzzzzzzzzz = @$_POST['addcomment'];
$viewread = @$_GET['viewread'];
if(empty($zzzzzzzzzzzzz) && ($viewread <> "true"))
{
    $url = $specurl;
}if(!empty($zzzzzzzzzzzzz))
{
    addcom();
    view2();
}
...
{
    $nums = 0;
}if(!empty($nums) || ($nums == 0) && ($ads <> ""))
{
    view2();
}
$url = $urlorig = mysql_real_escape_string($url, $link);
##############
...
function wordWrapIgnoreHTML($string, $length = 50, $wrapString = " ")
{
    ...
    if(!$coder)
    {
        $string = utfcoder2($string, 'UTF-8', 'windows-1251');
    }
    $wrapped = '';
    $word = '';
    $html = false;
    ...
    if(!$coder)
    {
        $wrapped = utfcoder2($wrapped, 'windows-1251', 'UTF-8');
    }
    return $wrapped;
}
...
function cuthtml($comment_msg)
{
    ...
    $comment_msg = htmlspecialchars($comment_msg, ENT_QUOTES);
    return $comment_msg;
}

function viewworld($msg, $kolvo)
{
    $msg = explode(" ", $msg);
    $i = 0;
    $out = "";
...
{
    global $idProduct, $massparam, $comment_msg, $names, $url, $date, $table, $wwp, $coder, $zzzzzzzzzzzzz, $langcommentit, $nums, $idcom, $divonter, $ratingcomment, $commentpx, $scravatar, $ratingval, $ratingcolor, $ratingznak, $icoservice, $staticcommentit;
    $i = 0;
    foreach($mass as $row)
    {
        if($row['level'] == 0)
        ...
        if(@$massparam['http'] == 1 && $viewhttp <> "")
        {
            ...
            {
                $names = '<a target="_blank" href="' . $viewhttp . '">' . $names . '</a>';
            }
            if($massparam['linknofol'] == 1)
            {
                $names = '<!--noindex--><a target="_blank" rel="nofollow" href="' . $viewhttp . '">' . $names . '</a><!--/noindex-->';
            }
        }

        $date = mont(date($massparam['formatdate'], strtotime($massparam['correcttime'], strtotime($row['data']))));
        //$date=mont(date('Y.m.d H:i:s',strtotime($massparam['correcttime'],strtotime($row['data']))));

        $commentpx = $massparam['pxlevel'] * $row['level'] + $massparam['startpx'];
        $scravatar = '';
        if($massparam['useravatar'] > 0)
        {
            @$scravatar = '<img src="' . str_replace($_SERVER['DOCUMENT_ROOT'], '', $textava[array_rand($textava = glob($_SERVER['DOCUMENT_ROOT'] . "/" . $wwp . "/avatars/" . "*.*"))]) . '" border="0" />';
        }
        $icoservice = '';
        if($massparam['loginzaglob'] == 1 && $massparam['loginzurl'] == 1 && $row['service'])
        {
            ...
            if($massparam['loginzaglob'] == 1 && $massparam['loginzavatar'] == 1 && $miniavatar)
            {
                $scravatar = '<img src="' . $miniavatar . '" border="0" alt="" title="" />';
            }

            if(preg_match('@yandex@smi', $row['service']))
            {
                $icoservice = '<img src="/' . $wwp . '/im/yandex.png" alt="Yandex" title="Yandex" />';
            }
            if(preg_match('@google@smi', $row['service']))
            {
                $icoservice = '<img src="/' . $wwp . '/im/google.png" alt="Google" title="Google Accounts" />';
            }
            if(preg_match('@vk\.com@smi', $row['service']))
            {
                $icoservice = '<img src="/' . $wwp . '/im/vkontakte.png" alt="Вконтакте" title="Вконтакте" />';
            }
            if(preg_match('@mail@smi', $row['service']))
            {
                $icoservice = '<img src="/' . $wwp . '/im/mailru.png" alt="Mail.ru" title="Mail.ru" />';
            }
            if(preg_match('@twitter@smi', $row['service']))
            {
                $icoservice = '<img src="/' . $wwp . '/im/twitter.png" alt="Twitter" title="Twitter" />';
            }
            if(preg_match('@loginza@smi', $row['service']))
            {
                $icoservice = '<img src="/' . $wwp . '/im/loginza.png" alt="Loginza" title="Loginza" />';
            }
            if(preg_match('@openid@smi', $row['service']))
            {
                $icoservice = '<img src="/' . $wwp . '/im/openid.png" alt="OpenID" title="OpenID" />';
            }
            if(preg_match('@myopenid@smi', $row['service']))
            {
                $icoservice = '<img src="/' . $wwp . '/im/myopenid.png" alt="MyOpenID" title="MyOpenID" />';
            }
            if(preg_match('@webmoney@smi', $row['service']))
            {
                $icoservice = '<img src="/' . $wwp . '/im/webmoney.png" alt="WebMoney" title="WebMoney" />';
            }
            if(preg_match('@facebook@smi', $row['service']))
            {
                $icoservice = '<img src="/' . $wwp . '/im/facebook.png" alt="Facebook" title="Facebook" />';
            }
            if(preg_match('@last@smi', $row['service']))
            {
                $icoservice = '<img src="/' . $wwp . '/im/lastfm.png" alt="Lastfm" title="Lastfm" />';
                $scravatar = str_replace('<img', '<img style="width:50px;height:50px;" ', $scravatar);
            }
        }
        ...
        if($massparam['rating'] == 1)
        {
            ...
            $ratingcomment = xparse("" . $_SERVER['DOCUMENT_ROOT'] . "/" . $wwp . "/skin/rating.php", '', get_defined_vars());
        }
        ...
        if($massparam['maxlevel'] > 0)
        {
            if($massparam['hideaddform'] == 1)
            {
                if((!$zzzzzzzzzzzzz))
                {
                    $divonter = xparse("" . $_SERVER['DOCUMENT_ROOT'] . "/" . $wwp . "/skin/replay.php", '', get_defined_vars());
                }
                else
                {
                    $divonter = "";
        ...
        if($massparam['maxlevel'] > 0)
        {
            $comment_msg = '<div id="z' . $idcom . '">' . $comment_msg . '</div>';
        }
        ...
        if($massparam['bbpanel'] == 1)
        {
            ...
            {
                $wpor = '';
                if($massparam['limitsizew'])
                {
                    $wpor = 'width="' . $massparam['limitsizew'] . '"';
                }
                $hpor = '';
                if($massparam['limitsizeh'])
                {
                    $hpor = 'height="' . $massparam['limitsizeh'] . '"';
                }
                if($massparam['picpars'] == 1)
                {
                    ...
                    foreach($tmp[0] as $n)
                    {
                        ...
                        {
                            $tmp2 = preg_replace("#\[img=(.*?)\]#si", "<img src=\"\\1\" $wpor $hpor border=\"0\" alt=\"\" title=\"\" />", $n);
                            $comment_msg = str_replace($n, $tmp2, $comment_msg);
                        }
                        else
                        {
                            $tmp2 = preg_replace("#\[img=(.*?)\]#si", "\\1", $n);
                            $comment_msg = str_replace($n, $tmp2, $comment_msg);
                        }
                    }
                }
                ...
                {
                    $comment_msg = preg_replace("#\[img=(.*?)\]#si", "<img src=\"\\1\" $wpor $hpor border=\"0\" alt=\"\" title=\"\" />", $comment_msg);
                }
            }
            ...
            if($massparam['justbb'] == 1)
            {
                ...
                $tmp[0] = str_replace(']', '', $tmp[0]);
                $spsjust = $tmp[0];
                if($spsjust <> 'center' || $spsjust <> 'right' || $spsjust <> 'left')
                    $spssize = 'left';
            ...
            if($massparam['colorbb'] == 1)
            {
                while(strpos($comment_msg, '[/COLOR]'))
                {
                    ...
                    $tmp[0] = strtolower($tmp[0]);
                    $tmp[0] = str_replace('[color=', '', $tmp[0]);
                    $oldcolor = str_replace(']', '', $tmp[0]);
                    $colorz = $masscolor[$oldcolor];
                    if(!$colorz)
                        $colorz = '#000';
            ...
            if($massparam['linkbb'] == 1)
            {
                ...
                if($massparam['linkpars'] == 1)
                {
                    ...
                    foreach($tmp[0] as $n)
                    {
                        if(preg_match("@" . $_SERVER['SERVER_NAME'] . "@smi", $n))
                        {
                            $tmp2 = preg_replace("#\[url=(.*?)\](.+?)\[/url\]#si", "<!--noindex--><a rel=\"nofollow\" target=\"_blank\" href=\"" . $massparam['linkredirect'] . "\\1\">\\2</a><!--/noindex-->", $n);
                            $comment_msg = str_replace($n, $tmp2, $comment_msg);
                            $tmp2 = preg_replace("#\[url=(.*?)\]\[/url\]#si", "<!--noindex--><a rel=\"nofollow\" target=\"_blank\" href=\"" . $massparam['linkredirect'] . "\\1\">\\1</a><!--/noindex-->", $n);
                            $comment_msg = str_replace($n, $tmp2, $comment_msg);
                            $tmp2 = preg_replace("#\[url\](.*?)\[/url\]#si", "<!--noindex--><a rel=\"nofollow\" target=\"_blank\" href=\"" . $massparam['linkredirect'] . "\\1\">\\1</a><!--/noindex-->", $n);
                            $comment_msg = str_replace($n, $tmp2, $comment_msg);
                        }
                        ...
                        {
                            $tmp2 = preg_replace("#\[url=(.*?)\](.+?)\[/url\]#si", "\\1", $n);
                            $comment_msg = str_replace($n, $tmp2, $comment_msg);
                            $tmp2 = preg_replace("#\[url=(.*?)\]\[/url\]#si", "\\1", $n);
                            $comment_msg = str_replace($n, $tmp2, $comment_msg);
                            $tmp2 = preg_replace("#\[url\](.*?)\[/url\]#si", "\\1", $n);
        ...
        if(isset($staticcommentit) || (@$_GET['static']))
        {
            $divonter = 'ffff';
        }

        $adminskinz = 'viewcom.php';
        if($admincom == 1)
        {
            $adminskinz = 'adminviewcom.php';
            $names = $massparam['nameadmin'];
            if($massparam['ratingadmin'] == 0)
            {
                $ratingcomment = '';
            }
        }
        ...
        if(sizeof($row['children']) > 0)
        {
            if($massparam['sort'] >= 3)
            {
                $row['children'] = array_reverse($row['children']);
            }
...
function view2()
{
    global $idProduct, $massparam, $comment_msg, $names, $url, $dataz, $table, $wwp, $coder, $zzzzzzzzzzzzz, $langcommentit, $nums, $staticcommentit, $link;
    ...
    $page = 0;
    $moderzap = '';
    if($massparam['moder'] == 1)
    {
        $moderzap = 'AND moder=0';
    }
    preg_match_all('#p-(.+?)-#is', $url, $arr);
    $idFromUrl = $arr[1][0];
    $resultget = mysql_query("SELECT * FROM `{$table}` WHERE url LIKE '%p-{$idFromUrl}-%' {$moderzap} ORDER BY {$dexrat} {$dex} ", $link);
    //var_dump("SELECT * FROM `{$table}` WHERE url LIKE '%p-'{$idProduct}'-%' {$moderzap} ORDER BY {$dexrat} {$dex} ", $link);
    

    while($myrow = mysql_fetch_array($resultget))
    {
        $bigmass[] = $myrow;
    }
    ...
    $pages = ceil($cnt / $rpp);
    if(($zzzzzzzzzzzzz) && ($massparam['sort'] == 0))
    {
        $nums = $pages - 1;
        if($nums < 0)
            $nums = 0;
    ...
    {
        echo '<div class="alert alert-info" role="alert">', $langcommentit['no_comments'], '</div>';
    }
...
##### Постраничная навигация #####
        ...
        if($pages > 0)
        {
            echo '<br />' . $langcommentit['skin_pages'];
        }
        if($page > 0)
        {
            // PREV

            $langcommentit['skin_pages_prev'] = '<span class="btn button-small circle prev">&lt;</span>';
            echo "<!-- <span style=\"cursor: pointer;\" onclick=\" var oldid=document.getElementById('oldid').value; makeRequest('/" . $wwp . "/func.php?viewread=true&ads=0&url=" . $url . "&static=" . $staticcommentit . "','ok',oldid)\">" . $langcommentit['skin_pages_first'] . "</span> |--> <span style=\"cursor: pointer;\" onclick=\" var oldid=document.getElementById('oldid').value; makeRequest('/" . $wwp . "/func.php?viewread=true&ads=" . ($page - 1) . "&url=" . $url . "&static=" . $staticcommentit . "','ok',oldid)\">" . $langcommentit['skin_pages_prev'] . "</span> ";
        }
        $start = $page - $rad;
        ...
        for($i = $start; $i < $end; $i++)
        {
            ...
            if($i == $page)
            {
                echo "<span class='btn unactive yellow button-small circle'>";
            }
            else
            {
                echo "<span class='btn button-small circle' onclick=\" var oldid=document.getElementById('oldid').value; makeRequest('/" . $wwp . "/func.php?viewread=true&ads=" . ($i) . "&url=" . $url . "&static=" . $staticcommentit . "','ok',oldid)\">";
            }
        ...
        if($pages > $links && $page < ($pages - $rad - 1))
        {
            ...
            echo " ... <span style=\"cursor: pointer;\" onclick=\" var oldid=document.getElementById('oldid').value; makeRequest('/" . $wwp . "/func.php?viewread=true&ads=" . ($pages) . "&url=" . $url . "&static=" . $staticcommentit . "','ok',oldid)\">[" . ($pages) . "]</span>";
        }

        if($page < $pages - 1)
        {
            // Next
            $langcommentit['skin_pages_next'] = '<span class="btn button-small circle next">&gt;</span>';
            echo " <span style=\"cursor: pointer;\" onclick=\" var oldid=document.getElementById('oldid').value; makeRequest('/" . $wwp . "/func.php?viewread=true&ads=" . ($page + 1) . "&url=" . ($url) . "&static=" . $staticcommentit . "','ok',oldid)\">" . $langcommentit['skin_pages_next'] . "</span> <!-- | <span style=\"cursor: pointer;\" onclick=\" var oldid=document.getElementById('oldid').value; makeRequest('/" . $wwp . "/func.php?viewread=true&ads=" . ($pages - 1) . "&url=" . $url . "&static=" . $staticcommentit . "','ok',oldid)\">" . $langcommentit['skin_pages_last'] . "</span> -->";
        }
##### Постраничная навигация #####
    }
    echo "</div>";
...
function addcom()
{
    global $massparam, $comment, $namenew, $url, $mailz, $dataz, $table, $wwp, $coder, $idcomnow, $zzzzzzzzzzzzz, $iconv, $captcha_keystring, $keystring, $hostsite, $hostrss, $langcommentit, $pass, $djos, $link;
    ...
    if($massparam['loginzaglob'] == 1)
    {
        ...
        if(empty($djos['error_message']))
        {
            if(preg_match('@vk\.com@smi', $djos['provider']))
            {
                if($djos['nickname'])
                {
                    $oldname = utfcoder($djos['nickname']);
                }
                else
                {
                    $oldname = utfcoder($djos['name']['first_name'] . ' ' . $djos['name']['last_name']);
                }
                $icoservice = $oldname . ' <img src="/' . $wwp . '/im/vkontakte.png" atl="" title="" border="0" />';
                $langcommentit['skin_name'] = '<img src="' . $djos['photo'] . '" atl="" title="" border="0" />';
            }

            if(preg_match('@twitter@smi', $djos['provider']))
            {
                $oldname = utfcoder($djos['nickname']);
                $icoservice = $oldname . ' <img src="/' . $wwp . '/im/twitter.png" atl="" title="" border="0" />';
                $langcommentit['skin_name'] = '<img src="' . $djos['photo'] . '" atl="" title="" border="0" />';
            }

            if(preg_match('@mail\.ru@smi', $djos['provider']))
            {
                $oldname = str_replace('http://openid.mail.ru/mail/', '', utfcoder($djos['identity']));
                $icoservice = str_replace('http://openid.mail.ru/mail/', '', $oldname) . ' <img src="/' . $wwp . '/im/mailru.png" atl="" title="" border="0" />';
            ...
            if(preg_match('@yandex@smi', $djos['provider']))
            {
                ...
                $icoservice = $oldname . ' <img src="/' . $wwp . '/im/yandex.png" atl="" title="" border="0" />';
                $langcommentit['skin_name'] = '';
            }
            ...
            if(preg_match('@myopenid\.com@smi', $djos['provider']))
            {
                ...
                $oldname = str_replace('.myopenid.com', '', $oldname);
                $icoservice = $oldname . ' <img src="/' . $wwp . '/im/myopenid.png" atl="" title="" border="0" />';
                $langcommentit['skin_name'] = '';
            }

            if(preg_match('@google@smi', $djos['provider']))
            {
                $oldname = utfcoder($djos['name']['first_name']);
                $icoservice = $oldname . ' <img src="/' . $wwp . '/im/google.png" atl="" title="" border="0" />';
                $langcommentit['skin_name'] = '';
            }


            if(preg_match('@loginza\.ru@smi', $djos['provider']))
            {
                $oldname = utfcoder($djos['nickname']);
                $icoservice = $oldname . ' <img src="/' . $wwp . '/im/loginza.png" atl="" title="" border="0" />';
            ...
            if(preg_match('@last\.fm@smi', $djos['provider']))
            {
                ...
                {
                    $oldname = utfcoder($djos['name']['full_name']);
                }
                $icoservice = $oldname . ' <img src="/' . $wwp . '/im/lastfm.png" atl="" title="" border="0" />';
                $langcommentit['skin_name'] = '<img style="width:50px;height:50px;"  src="' . $djos['photo'] . '" atl="" title="" border="0" />';
            }
            ...
            if(preg_match('@webmoney@smi', $djos['provider']))
            {
                $oldname = utfcoder($djos['identity']);
                $oldname = str_replace('http://', '', $oldname);
                $oldname = str_replace('.webmoney.ru/', '', $oldname);
                $icoservice = $oldname . ' <img src="/' . $wwp . '/im/webmoney.png" atl="" title="" border="0" />';
                $langcommentit['skin_name'] = '';
            }
            ...
            if(preg_match('@facebook\.com@smi', $djos['provider']))
            {
                ...
                $icoservice = $oldname . ' <img src="/' . $wwp . '/im/facebook.png" atl="" title="" border="0" />';
                $langcommentit['skin_name'] = '<img src="' . $djos['photo'] . '" atl="" title="" border="0" />';
            }
    ...
    // Оценка товара
    $mark = empty($_POST['mark']) ? 5 : intval($_POST['mark']);
    if(!$mark || $mark < 0 || $mark > 5) {
        $mark = 5;
    }
    
    // ID товара
    $products_id = filter_input(INPUT_POST, 'products_id', FILTER_VALIDATE_INT, array('min_range' => 1));
...
//Блок проверки на сервере, чтобы случайно не попал спам, если даже был обход Ajax-а
    ...
    if($usmail)
    {
        if(!preg_match("/^[A-Z0-9._%-]+@[A-Z0-9][A-Z0-9.-]{0,61}[A-Z0-9]\.[A-Z]{2,6}$/i", $usmail))
        {
...
//-------------------------------------
    ...
    {
        $comment = str_replace("\n", "<br />", $comment);
        while((preg_match('/(<br[^>]*>\s*)(<br[^>]*>\s*)(<br[^>]*>\s*)/i', $comment)) == true)
        {
            $comment = preg_replace('/(<br[^>]*>\s*)(<br[^>]*>\s*)(<br[^>]*>\s*)/i', '<br />', $comment);
        }
        ...
        if(!$dieadd)
        {
            if($massparam['loginzaglob'] == 0)
            {
                $namenew = utfcoder($namenew);
            }
            if($massparam['loginzaglob'] && $massparam['blockeasy'] && !empty($djos['error_message']))
            {
                $namenew = utfcoder($namenew);
            }
            ...
            {
                $comment = substr($comment, 0, ($massparam['sumvl']) * 2);
            }
            if(function_exists('mb_substr'))
            {
            ...
            {
                $namenew = substr($namenew, 0, ($massparam['sumvlname']) * 2);
            }
            $namenew = htmlspecialchars(strip_tags($namenew));
            $dataz = strip_tags($dataz);
            $usmail = str_replace("htt://", "", $usmail);
            $usmail = strip_tags($usmail);
            $usurl = strip_tags($usurl);

            $query = "INSERT INTO `$table` (`num`, `url`, `name`, `comm`, `data`, `moder`, `ip`, `mail`,`rootid`,`level`,`http`,`service`,`avatar`,`admincom`,`raitng`,`iprating`, `mark`, `products_id`) VALUES (NULL, '" . $url . "', '" . mysql_real_escape_string($namenew, $link) . "', '" . mysql_real_escape_string($comment, $link) . "', '" . $dataz . "', '" . $massparam['moder'] . "', '" . getipcomment() . "', '" . mysql_real_escape_string($usmail, $link) . "','" . $idcomnow . "','" . $level . "','" . mysql_real_escape_string($usurl, $link) . "','" . $service . "','" . $avatar . "','" . $admincom . "',0,'', {$mark}, " . ($products_id ? $products_id : 'NULL') . ");";
            $sort = @mysql_query($query, $link) or die("$query");
            
            // Если ID товара указан, то обновляем количество комментариев для данного товара и средний рейтинг
            if($products_id) {
                // Проверяем, что такой товар действительно существует
                $query = tep_db_query("SELECT COUNT(*) AS `count` FROM products WHERE products_id = {$products_id} LIMIT 1");
                $result = tep_db_fetch_array($query);
                if($result['count']) {
                    $query = mysql_query("SELECT COUNT(num) AS `count`, SUM(mark) `mark` FROM road WHERE products_id = {$products_id} GROUP BY products_id", $link);
                    if(mysql_num_rows($query)) {
                        $result = mysql_fetch_assoc($query);
                        $result['mark'] = ceil($result['mark'] / $result['count']);
                        tep_db_query("UPDATE products SET products_comments_count = {$result['count']}, products_rating = {$result['mark']} WHERE products_id = {$products_id}");
                    }
                }
            }
            ...
            if($massparam['mail'] == 1 && $admincom != 1)
            {
                ...
                {
                    $smail->build_message('u');
                }
                foreach($mail as $n)
                {
                    if($n)
                    {
                        $smail->send($_SERVER['SERVER_NAME'], $n, "robocommentit@" . $_SERVER['HTTP_HOST'], $langcommentit['mail_title']);
                    }
                }
            }
        ...
        {
            if($massparam['europing'] == 1)
            {
                pingmagnetik($massparam['titlecia'], $hostsite);
            }
            if($massparam['yaping'] == 1)
            {
                pingyandex($massparam['titlecia'], $hostrss);
            }
        }
    }
}

function utfcoder($txt)
{
    global $iconv, $coder;
    if($coder == 1)
    {
        if($iconv <> 0)
        {
            $txt = iconv('UTF-8', 'windows-1251//IGNORE', $txt);
        }
        else
        {
            $txt = Utf8Win($txt, 'w');
...
function utfcoder2($txt, $in = 'UTF-8', $out = 'windows-1251')
{
    global $iconv, $coder;
    if($iconv <> 0)
    {
        $txt = iconv($in, $out . '//IGNORE', $txt);
    } return $txt;
}

function viewform()
{
    global $massparam, $url, $urlorig, $table, $wwp, $coder, $idcomnow, $zzzzzzzzzzzzz, $iconv, $langcommentit, $smilebar, $htmlz, $rssico, $width, $height, $novis, $novisurl, $titlenter, $novismail, $captsourse, $oldname, $oldmess, $oldmail, $autoarea, $bbpanel, $panelbar, $linkbutton, $picbutton, $quotebutton, $colorbutton, $sizebutton, $justbutton, $capt, $sumvlname, $oldurl, $djos, $icoservice, $miniavatar, $novisnick, $exitbut;
    $sumvlname = $massparam['sumvlname'];
    $autoarea = '';
    if($massparam['autosize'] == 1)
    {
        $autoarea = 'onkeyup="autosize(this)" onfocus="autosize(this)"';
    }
    preg_match_all('#p-(.+?)-#is', $url, $arr);
    $idFromUrl = $arr[1][0];
     
    $countcomment = mysql_query("SELECT count(url) FROM `$table` WHERE url='" . mysql_real_escape_string('%p-'.$idFromUrl.'-%', $link) . "'", $link);
    $out = mysql_result($countcomment, 0);
    if(!$out)
        $langcommentit['skin_post_comment'] = $langcommentit['skin_first_post'];
    ...
    if($massparam['loginzaglob'] == 1)
    {
        ...
        if(empty($djos['error_message']))
        {
            ...
            {
                $oldname = str_replace('http://openid.mail.ru/mail/', '', utfcoder($djos['identity']));
                $icoservice = $oldname . ' <img src="/' . $wwp . '/im/mailru.png" atl="" title="" border="0" />';
                $langcommentit['skin_name'] = '';
            }
...
<a title="$langcommentit[editor_smile]" href="javascript: showhide('smilebar');" id="objcomm2" class="pic13"></a>
...
EOF;
    ...
    {
        $novis = 'type="text"';
        $capt = xparse("" . $_SERVER['DOCUMENT_ROOT'] . "/" . $wwp . "/skin/capt.php", '', get_defined_vars());
    }
    else
    {
        $titlenter = '';
    ...
    if($massparam['viewentermail'] == 1)
    {
        $novismail = 'type="text"';
    }
    else
    {
        $novismail = 'type="hidden"';
        $langcommentit['skin_mail'] = '';
    }
    if($massparam['http'] == 1)
    {
        $novisurl = 'type="text"';
    ...
    {
        $novisurl = 'type="hidden"';
        $langcommentit['skin_homepage'] = '';
    }
    if($massparam['rss'] == 1)
    {
        $rssico = '<a href="/' . $wwp . '/rss.php?url=' . $urlorig . '"><img alt="RSS" title="RSS" src="/' . $wwp . '/im/rss.png" border="0" /></a>';
    }
    ...
    echo xparse("" . $_SERVER['DOCUMENT_ROOT'] . "/" . $wwp . "/skin/addcom.php", '', get_defined_vars());
}
...
function pingmagnetik($blog_name, $blog_link)
{
    ...
    $methodcall_schema = '<methodCall>' . "\n";
    $methodname_line = '<methodName>weblogUpdates.ping</methodName>' . "\n";
    $params_schema = '<params>' . "\n";
    $blogname_line = '<param><value>' . $blog_name . '</value></param>' . "\n";
    $bloglink_line = '<param><value>' . $blog_link . '</value></param>' . "\n";

    $params_end = '</params>' . "\n";
    $methodcall_end = '</methodCall>' . "\n";
    $ping_url = 'http://rpc.pingomatic.com';
    $ping_post_content = $xml_schema . $methodcall_schema . $methodname_line;
    $ping_post_content .= $params_schema . $blogname_line . $bloglink_line;
    $ping_post_content .= $params_end . $methodcall_end;
    $connect = curl_init();
    ...
    curl_setopt($connect, CURLOPT_POSTFIELDS, $ping_post_content);
...
class html_mime_mail
{
    ...
    function send($server, $to, $from, $subject = "", $headers = "")
    {
        ...
        mail($to, $subject, $this->mime, $this->headers . $headers . "\n");
    }

}

function commentrating($comment, $col)
{
    global $table, $link;
    if($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')
    {
        ...
        $col = intval($col);
        $SQL = "SELECT raitng,iprating FROM `$table` WHERE num='$comment' LIMIT 1";
        $result = mysql_query($SQL, $link);
        if(mysql_num_rows($result))
        {
            $row = mysql_fetch_array($result);
        }
        if(!preg_match('@' . getipcomment() . '@smi', $row['iprating']))
        {
            if($col == 1)
            {
                $col = '+1';
                $row['raitng'] = $row['raitng'] + 1;
            }
            else
            {
                $col = '-1';
            ...
            $query = "UPDATE `$table` SET raitng=raitng" . $col . ", iprating='" . $row['iprating'] . getipcomment() . "|' WHERE num='$comment' LIMIT 1;";
            $sort = @mysql_query($query, $link) or die("$query");
            if($row['raitng'] > 0)
                $row['raitng'] = '+' . $row['raitng'];
            echo $row['raitng'];
        }
        else
        {
...
function loginza()
{
    global $table, $specurl, $langcommentit, $panellozinza, $wwp, $massparam;
    $provader = '';
    $provaderurl = '';
    $urlxlogiza = rawurlencode('http://' . $_SERVER['HTTP_HOST'] . $specurl);
    if($massparam['yandex'] == 1)
    {
        $provader.='<img src="/' . $wwp . '/im/yandex.png" alt="Yandex" title="Yandex" />';
        $provaderurl.='yandex,';
    }
    if($massparam['google'] == 1)
    {
        $provader.='<img src="/' . $wwp . '/im/google.png" alt="Google" title="Google Accounts" />';
        $provaderurl.='google,';
    }
    if($massparam['vkontakte'] == 1)
    {
        $provader.='<img src="/' . $wwp . '/im/vkontakte.png" alt="VK" title="VK" />';
        $provaderurl.='vkontakte,';
    }
    if($massparam['mailru'] == 1)
    {
        $provader.='<img src="/' . $wwp . '/im/mailru.png" alt="Mail.ru" title="Mail.ru" />';
        $provaderurl.='mailru,';
    }
    if($massparam['twitter'] == 1)
    {
        $provader.='<img src="/' . $wwp . '/im/twitter.png" alt="Twitter" title="Twitter" />';
        $provaderurl.='twitter,';
    }
    if($massparam['loginza'] == 1)
    {
        $provader.='<img src="/' . $wwp . '/im/loginza.png" alt="Loginza" title="Loginza" />';
        $provaderurl.='loginza,';
    }
    if($massparam['myopenid'] == 1)
    {
        $provader.='<img src="/' . $wwp . '/im/myopenid.png" alt="MyOpenID" title="MyOpenID" />';
        $provaderurl.='myopenid,';
    }
    if($massparam['openid'] == 1)
    {
        $provader.='<img src="/' . $wwp . '/im/openid.png" alt="OpenID" title="OpenID" />';
        $provaderurl.='openid,';
    }
    if($massparam['webmoney'] == 1)
    {
        $provader.='<img src="/' . $wwp . '/im/webmoney.png" alt="WebMoney" title="WebMoney" />';
        $provaderurl.='webmoney,';
    }
    if($massparam['facebook'] == 1)
    {
        $provader.='<img src="/' . $wwp . '/im/facebook.png" alt="Facebook" title="Facebook" />';
        $provaderurl.='facebook,';
    }
    if($massparam['lastfm'] == 1)
    {
        $provader.='<img src="/' . $wwp . '/im/lastfm.png" alt="Lastfm" title="Lastfm" />';
        $provaderurl.='lastfm,';
    }
    $panellozinza = '<script src="http://loginza.ru/js/widget.js" type="text/javascript"></script><a href="https://loginza.ru/api/widget?token_url=' . $urlxlogiza . '&providers_set=' . $provaderurl . '" class="loginza">' . $provader . '</a>';
    echo xparse("" . $_SERVER['DOCUMENT_ROOT'] . "/" . $wwp . "/skin/loginza.php", '', get_defined_vars());
}