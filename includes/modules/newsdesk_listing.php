...
?>
	...
	<tr>
		<td>
<br /><br />
<table align="center" width="100%" cellspacing="0" border="0" cellpadding="2">
	<tr>
		<td class="smallText">&nbsp;<?php echo $listing_split->display_count(TEXT_DISPLAY_NUMBER_OF_ARTICLES); ?>&nbsp;</td>
		<td align="right" class="countPages smallText">&nbsp;<?php echo TEXT_RESULT_PAGE . ' ' . $listing_split->display_links(MAX_DISPLAY_NEWSDESK_PAGE_LINKS, tep_get_all_get_params(array('page', 'info', 'x', 'y'))); ?>&nbsp;</td>
	</tr>
</table>
  ...
  $listing_query = tep_db_query($listing_split->sql_query);
    while ($listing = tep_db_fetch_array($listing_query)) {
      $rows++;*/
        ...
        $colsp = 1;
        if ($listing_values['newsdesk_image'] != '') {
         $colsp = 2;
        }
		...
		case 'NEWSDESK_ARTICLE_NAME':
            $lc_params = '';
			$lc_text = '<h1><a href="' . tep_href_link(FILENAME_NEWSDESK_INFO, ($newsPath ? 'newsPath=' . $newsPath . '&' : '') 
			. 'newsdesk_id=' . $listing_values['newsdesk_id']) . '">' . $listing_values['newsdesk_article_name'] . '</a></h1>';
			break;
       		case 'NEWSDESK_DATE_AVAILABLE':
			$lc_text = '';
			break;
		case 'NEWSDESK_ARTICLE_SHORTTEXT':
            $lc_params = '';
			$lc_text = '<div class="date">'.tep_date_short_custom($listing_values['newsdesk_date_added'],true) . '</div>';
                       
           	$lc_text .= '<div class="newsdesk_article_shorttext"><a href="' . tep_href_link(FILENAME_NEWSDESK_INFO, ($newsPath ? 'newsPath=' . $newsPath . '&' : ''). 'newsdesk_id=' . $listing_values['newsdesk_id']) . '">' . $listing_values['newsdesk_article_shorttext'].'</div>';
			break;
		case 'NEWSDESK_ARTICLE_DESCRIPTION':
			$lc_text = '<a href="' . tep_href_link(FILENAME_NEWSDESK_INFO, ($newsPath ? 'newsPath=' . $newsPath . '&' : '') 
...
if (($listing_values['newsdesk_image'] != '') && ($listing_values['newsdesk_image'] != 'Array')) {
			$lc_align = 'left';
            $lc_params = '';
			$lc_text = '<td width="120"><a href="' . tep_href_link(FILENAME_NEWSDESK_INFO, ($newsPath ? 'newsPath=' . $newsPath . '&' : '') 
			. 'newsdesk_id=' . $listing_values['newsdesk_id']) . '">' . tep_image(DIR_WS_IMAGES . $listing_values['newsdesk_image'], 
			$listing_values['newsdesk_article_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a></td>';
} else {
			$lc_align = 'center';
			$lc_text = '&nbsp;';
}
