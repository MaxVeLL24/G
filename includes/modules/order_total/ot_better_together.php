 ...
 */
    ...
    function get_discount_info($id) {
       ...
       for ($dis=0, $n=count($this->twoferlist); $dis<$n; $dis++) {
           ...
           if ($match == 1) {
              ...
              $product_info_query = tep_db_query("select pd.products_name, p.products_images from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_status = '1' and p.products_id = '" . (int)$id . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "'");
       ...
       for ($dis=0, $n=count($this->discountlist); $dis<$n; $dis++) {
           ...
           if ( ($li->flavor == PROD_TO_PROD) &&
              ...
              $product_better_query = tep_db_query("select pd.products_name, p.products_images from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_status = '1' and p.products_id = '" . (int)$li->ident2 . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "'");
              ...
              $image_file_name = explode(';', $product_info['products_images']);
              $image_file_name_better = explode(';', $product_better['products_images']);
              $disc_link = '<div class="left item">
                              <div class="name_ot_better">
                                <a href="'.tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $li->ident1).'">'.$product_info['products_name'].'</a>
                              </div>
                              <div class="image">
                                <a href="'.tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $li->ident1).'"><img height="100" src="/images/'. $image_file_name[0].'" /></a>
                              </div>
                            </div>
                            <div class="left">
                            ...
                            <div class="left item">
                              <div class="name_ot_better">
                                <a href="'.tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $li->ident2).'">'.$product_better['products_name'].'</a>
                              </div>
                              <div class="image"><a href="'.tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $li->ident2).'"><img height="100" src="/images/'. $image_file_name_better[0].'" /></a></div>
                            </div>';
  ...
  if($_POST['products_id']=='') {
      $xsell_query = tep_db_query('select x.products_id, x.xsell_id, x.discount from ' . TABLE_PRODUCTS . ' p, ' . TABLE_PRODUCTS_XSELL . ' x where x.products_id = p.products_id and x.discount!=""');
